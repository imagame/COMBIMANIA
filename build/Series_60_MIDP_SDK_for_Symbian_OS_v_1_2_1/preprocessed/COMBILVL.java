...
public void CargarNivel() {
  ...
  if(ObtIdLvl()){ //Si el nivel ha cambiado
    ...
    NIVmap_Cargar();
    HUDstr = TXT_Cargar(); //Carga texto de la página del nivel
  }
}
...
private String TXT_Cargar()
{
    int idxFas=0;    
    StringBuffer strbuf=null;
    
    //Cargar configuración (offsets de cada texto de fase y nivel en TXT.bin
    if(TXTconfPag==null){
    ...
    //Cargar stringbuffer con texto para la página <idPag> correspondiente al nivel 
    if(idModo!=0 || (idModo==0 && idNiv==0)){ //En modo tutorial sólo aplica al primer nivel
        try {
            DataInputStream dis = new DataInputStream(getClass().getResourceAsStream("/TXT.bin"));
            if (idModo == 0)
            ...
            //se situa en la página inicial de texto del modo
            for (int i = 0; i < TXTconfPag[idxFas];i += dis.skipBytes(TXTconfPag[idxFas] - i));
            strbuf = new StringBuffer(dis.readUTF());
            dis.close();
        }
        catch (Exception ex) {
            System.out.println("EX - TXT_Cargar(fase)");
    ...
    if(idPag !=-1){
            try {
                DataInputStream dis = new DataInputStream(getClass().getResourceAsStream("/TXT.bin"));
                //se situa en la página inicial del nivel
                for (int i = 0; i < TXTconfPag[idPag]; i += dis.skipBytes(TXTconfPag[idPag] - i));
                if(idModo==0 && idNiv>0)			
                    strbuf = new StringBuffer(dis.readUTF());
                else{
                    strbuf.append('\n');
                    strbuf.append(dis.readUTF());
                }
                dis.close();
             }
             catch (Exception ex) {
