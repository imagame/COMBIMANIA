...
//#            public static final byte NUMPIXELS_TILEY = 8;
           ...
           public static final short GAMw = NUMTILES_SCRX * NUMPIXELS_TILEX;
           public static final short GAMh = NUMTILES_SCRY * NUMPIXELS_TILEY;                      
           public static short GAMwLe = 0;
           public static short GAMwRi = 0;
...
//#            public static final int FNTW = 6; //Ancho fuente
           ...
           //#endif
            ...
            //#if CFGtamanyoGfx == 2
            public static final short MENhUp = 64;
            public static final short MENhDo = 16;
            public static final short MENhOp = 18;
            //#else            
//#             public static final short MENhUp = 28;
//#             public static final short MENhDo = 12;
//#             public static final short MENhOp = 12;                       
            //#endif
...
private void GAM_InitPausa()
{
    TIM_IniPausa();
    GAMsts = PLAY_OUT_PAUSA;
    MENid = MENop = 0;
    TXTiniPos = 0;
    TXTnumCars = TXT_ObtLongTextoFwd(LVL.HUDstr, 0, 100, DEVw-12);
}
    ...
    case PLAY_OUT_PAUSA:
         ...
         if(actorAccion == DER && (MENid==0 || MENid==1)){ //[PTE: Parche MENid0
           ...
           if(MENid>0){             
             if(MENid==2){
                ...
                TXTnumCars = TXT_ObtLongTextoFwd(LVL.HUDstr, 0, 100, DEVw-12);
                 ...
                 TXTnumCars = TXT_ObtLongTextoBwd(LVL.HUDstr, TXTiniPos-1, 100, DEVw-12);
           ...
           if(MENid<2){
             MENid++;
             MENop=0;
           }else if((TXTiniPos+TXTnumCars)<LVL.HUDstr.length()){
                MENid++;
                MENop=0;  
                TXTiniPos += TXTnumCars;
                TXTnumCars = TXT_ObtLongTextoFwd(LVL.HUDstr, TXTiniPos, 100, DEVw-12);             
           }
...
//Datos HUD y pantalla texto intro
    ...
    TXTiniPos = 0;
    TXTnumCars = TXT_ObtLongTextoFwd(LVL.HUDstr, 0, 56, DEVw-12);
...
//Dibujo de HUD estático
   ...
   //System.out.println(" => GAM_HUD_static");
      ...
      g.setClip(0,nivYini, DEVw, DEVh - nivYini);
      g.setColor(HUDcol);
      g.fillRect(0, nivYini, DEVw, DEVh - nivYini);
      HUD_DibImg(g,0,nivYini,31);
      HUD_DibImg(g,DEVw-8,nivYini,32);
      HUD_DibImg(g,0,DEVh-8,33);
      HUD_DibImg(g,DEVw-8,DEVh-8,34);
      for (int i = 0; i < (DEVw - 16) / 8; i++) {
        HUD_DibImg(g,(i<<3)+8,nivYini,37);
        HUD_DibImg(g,(i<<3)+8,DEVh - 2,38);
      }
      for (int j = 0; j < (DEVh - nivYini - 16) / 8; j++) {
        HUD_DibImg(g,0,(j<<3)+nivYini+8,35);
        HUD_DibImg(g,DEVw-2,(j<<3)+nivYini+8,36);
      }
...
private void HUD_DibInOut(Graphics g){
     ...
     g.fillRect(0, 14 * NUMPIXELS_TILEY, DEVw, DEVh - 14 * NUMPIXELS_TILEY);
...
private void HUD_DibIni(Graphics g){
     ...
     g.setClip(0, 4, DEVw, 110);
     FX_DibRect(g, DEVw / 2 - 5 * NUMPIXELS_TILEX,
                DEVw / 2 + 5 * NUMPIXELS_TILEX, 8, (short) 8 + 100);

     //Dibujar tipos de piezas del nivel
     g.setClip(0, nivY, DEVw, 14);
     g.setFont(fSbt);
     g.setColor(0x224466);
...
private void HUD_DibEnd(Graphics g){
     ...
     FX_DibRect(g, DEVw / 2 - 5 * NUMPIXELS_TILEX,
                DEVw / 2 + 5 * NUMPIXELS_TILEX, 32,
                (short) 32 + (LVL.idModo == 1 ? 90 : 40) + (GAMbRecord ? 16 : 0));
     if (GAMbRecord) {
       g.setClip(0, nivY - 4, DEVw, 16);
       MEN_DibRect(g, 32, nivY - 4, DEVw - 32, nivY + 14, false,
                   0x90336699, 0);
       g.setColor(0xFFFFFF);
       g.setFont(fSbt);
       g.drawString(GAMstr[16], DEVw / 2, nivY - 1,
...
private void HUD_DibModCursor(Graphics g, short pnivX, short pnivY, boolean pant){
     ...
     for (byte i = 0; i < 5; i++){
       ...
       g.setColor(0x224466);
       g.setFont(fSbt);
       g.drawString(CURcont[i]==-1?"--":Integer.toString(CURcont[i]),DEVw-32,y+1,Graphics.TOP | Graphics.RIGHT);
       g.setColor(0x738AAB);
       g.drawLine(pnivX+16, y+10, pant?pnivX+50:DEVw-62, y+10);
     }
}
...
private void HUD_DibPausa(Graphics g){
    ...
    g.fillRect(1,1,DEVw-2,18);
    //Marco cabecera
    g.setColor(0x3B465A); //azul fuerte
    g.drawRect(0, 0, DEVw-1, 19);
    //Imprimir título Nivel
    g.setFont(fTit);
    g.setColor(0x224466);
    g.drawString(GAMstr[LVL.idxStrTit],4,4,Graphics.TOP | Graphics.LEFT);
    g.drawString("Nivel:"+ Integer.toString(LVL.idNiv+1), DEVw-4, 4, Graphics.TOP | Graphics.RIGHT);

    //Pie
    if (MENid >= 2 || MENid==-1)
      MEN_DibRect(g, 1, 43, DEVw - 2, 189, true, 0xFFA6BDD3, 0xFF738AAB);
    else
      MEN_DibRect(g, 8, 148, DEVw - 9, 189, true, 0xFFA6BDD3, 0xFF738AAB);
    if(MENid==-1){ //confirmación de abandono de partida
      MEN_DibKeys(g, (byte) 6);
      g.setFont(fCnt);
      g.setColor(0x224466);
      g.drawString(GAMstr[31], DEVw / 2, 78, Graphics.TOP | Graphics.HCENTER);
      return;
    }
    else{
      MENmsg = -1;
      ...
      HUD_DibImg(g, 84, 200, (MENid>=2 && ((TXTiniPos+TXTnumCars) >= LVL.HUDstr.length()))? 81 : 30);
    ...
    g.fillRect(0,20,DEVw,19);
    g.setFont(fSbt);
    g.setColor(0x224466);
    g.drawString(GAMstr[MENpauCnf[(MENid>2?2:MENid)*4+3]],DEVw/2,23,Graphics.TOP | Graphics.HCENTER);
   ...
   if (MENid == 0){
    ...
    TXT_DibLineasString(g, LVL.HUDstr, TXTiniPos, TXTnumCars, 6, 46, DEVw-12, -1);
}
...
//#    TXT_DibLinea(g,pfase != -1 ? "Tiempo Record:" : "Tiempo Nivel:", pfase!=-1?24:32, py,-1);
   ...
   if (pfase != -1 && COMBIMidlet.M.MENtimRecord[pniv] == 0){
    //#if CFGfnt<2    
    g.drawString("--:--", DEVw - 24, py,Graphics.TOP | Graphics.RIGHT);
    //#else
//#     TXT_DibLinea(g, "--:--", DEVw - 24, py, 1);   
    //#endif
   }
   else {
     ...
     g.drawString( (min < 10 ? "0" : "") + Integer.toString(min) + ":" +
                  ...
                  DEVw-(pfase!=-1?24:32), py, Graphics.TOP | Graphics.RIGHT);
     //#else
//#      TXT_DibLinea(g, (min < 10 ? "0" : "") + Integer.toString(min) + ":" +
//#                   (sec < 10 ? "0" : "") + Integer.toString(sec),
//#                   DEVw-(pfase!=-1?24:32), py, 1);   
     //#endif
   }
 }
...
//----------------------------------------------------------------- HUD_DibPun
 private void HUD_DibPun(Graphics g, short py, byte pRec){
    ...
    g.drawString(Integer.toString((pRec==1?COMBIMidlet.M.MENscoreRecord:GAMscore)),DEVw-(pRec==1?24:32),py,Graphics.TOP | Graphics.RIGHT);
    //#else
//#     TXT_DibLinea(g,pRec==1?"Record Puntos:":"Puntos:", pRec==1?24:32, py, -1);   
//#     TXT_DibLinea(g,Integer.toString((pRec==1?COMBIMidlet.M.MENscoreRecord:GAMscore)), DEVw-(pRec==1?24:32), py, 1);      
    //#endif
 }
...
//#     g.drawString("Nivel: "+Integer.toString(LVL.idNiv+1), finX-2, iniY+4, Graphics.TOP | Graphics.RIGHT);
    ...
    //#if CFGfnt==0
//#     TXT_DibLineasString(g, LVL.HUDstr, TXTiniPos, TXTnumCars, 6, iniY + NUMPIXELS_TILEY + 8, DEVw-12, -1);
    //#else
    //TXT_DibLineasString(g, LVL.HUDstr, TXTiniPos, TXTnumCars, DEVw/2, iniY + NUMPIXELS_TILEY + 8, DEVw-12, 0);
    TXT_DibLineasString(g, LVL.HUDstr, TXTiniPos, TXTnumCars, 6, iniY + NUMPIXELS_TILEY + 8, DEVw-12, -1);
    //#endif
    ...
    if(TXTsig)
      HUD_DibImg(g,6,DEVh-12,30);
    if(TXTant)
      HUD_DibImg(g,DEVw-14,DEVh-10,29);
}
...
private void HUD_SoftLeft(){
    if(GAMbNivIntro){
      if (!TXTsig) { //si estado es CONTINUAR por llegar a final de texto
        ...
        {
            TXTiniPos += TXTnumCars;
            TXTnumCars = TXT_ObtLongTextoFwd(LVL.HUDstr, TXTiniPos, 56, DEVw-12);
...
private void HUD_SoftRight(){
    if(GAMbNivIntro){
        if(TXTant){
          TXTnumCars = TXT_ObtLongTextoBwd(LVL.HUDstr, TXTiniPos-1, 56, DEVw-12);
	  TXTiniPos -= TXTnumCars;
	  TXTsig = true;
...
private int TXT_ObtLongTextoFwd(String pStr, int pPosIni, int pLimAlto, int pLimAncho)
{
	...
	int numlinMax = pLimAlto/FNTH;
        //#else
//#         int numlinMax = pLimAlto/fCnt.getHeight();
        //#endif
	int numlin=0;
...
//-------------------------------------------------------------- TXT_CalcLong
// Desc: Calcula longitud en pixels del string <pStr>
//-----------------------------------------------------------------------------
...
//#if CFGfnt != 0
//--------------------------------------------------------------- TXT_DibLinea
// Desc: Dibuja string <pStr> en posiciones globales <px>, <py>
...
//----------------------------------------------------------------- TXT_DibCar
// Desc: Dibuja caracter <c> en posiciones globales <px>, <py>
//----------------------------------------------------------------------------
...
//Definición de variables para colocación correcta de pantalla   
       ...
       if(DEVw == 176){ 
           ...
           GAMscrPatchLE=0;
           if(DEVh < 160){ //ALtura mínima para que quepa el HUD entero
               HUD_RI = 1;
               GAMwLe = 0;
               GAMwRi = (short)(DEVw - GAMw);
           }
           else          
            GAMwLe = GAMwRi = (short)((DEVw - GAMw)/2);
       }
...
private void LOA_Dibujar(Graphics g){
   if (GAMbsplash){
     ...
     g.drawImage(image, DEVw/2,20,Graphics.TOP | Graphics.HCENTER);
     image = null;
     System.gc();
     GAMbsplash = false;
   }
   ...
   if (GAMsts == 0){
     g.setColor(0x606060);
    //#if CFGtamanyoGfx == 2
     g.drawString(GAMstr[29],DEVw/2,180,Graphics.TOP | Graphics.HCENTER);
     g.drawString(GAMstr[37],30,160,Graphics.TOP | Graphics.LEFT);
     //#else
//#      g.drawString(GAMstr[29],DEVw/2,DEVh,Graphics.BOTTOM | Graphics.HCENTER);
...
//Dibujar barra de progreso en función de nº de grafico cargado
     ...
     g.setColor(0xFFFFFF);
     //#if CFGtamanyoGfx == 2
     g.fillRect(0,160,DEVw,DEVh-160);

     if((MENid = (byte)((MENid+1)%10))==0)
       VISbUpd=!VISbUpd;
     g.setColor(VISbUpd?0xe0e0e0:0x000000);
     g.setFont(fCnt);
     g.drawString(GAMstr[38], DEVw / 2, 170, Graphics.TOP | Graphics.HCENTER);
     g.setFont(fSbt);
     g.setColor(0x224466);
     g.drawString("www.imagame.com", DEVw / 2, 190, Graphics.TOP | Graphics.HCENTER);
     //#else
//#          g.drawString(GAMstr[38],DEVw / 2,118,Graphics.BOTTOM | Graphics.HCENTER);
     //#endif
   }
}
...
private void MEN_Dibujar(Graphics g){
    ...
    g.fillRect(5,MENhUp+4,DEVw-9,MENh);
    
    g.setColor(0x3B465A); //color azul fuerte bordes pantalla y bordes area menu
    g.drawRect(0,0,DEVw-1,DEVh-MENhDo-1); 
    g.drawRect(4,MENhUp+3,DEVw-9,MENh); //(64+4+16+5-1)
    ...
    g.drawImage(imgCab, DEVw/2, 1, Graphics.TOP | Graphics.HCENTER);
    //#else
//#      g.setColor(0x661100);
//#      g.setFont(fTit);    
//#      g.drawString("COMBImanía",DEVw/2,2,Graphics.TOP | Graphics.HCENTER);
    //#endif
    ...
    //#if CFGtamanyoGfx == 2
     MEN_DibRect(g,DEVw/2-22,42, DEVw-5, 62, false, 0xB0336699, 0);
    //#else
//#      MEN_DibRect(g,4,15, DEVw-5, 29, false, 0xB0336699, 0);
    //#endif
     g.setColor(0xFFFFFF);
    //#if CFGtamanyoGfx == 2
     g.setFont(MENid==3?fSbt:fTit);
     g.drawString(GAMstr[MENid==0?6:MENid-1], (DEVw-5 - (DEVw/2-22))/2+(DEVw/2-22), 57,  Graphics.BASELINE | Graphics.HCENTER);
    //#else
//#      g.setFont(fTit);
//#      g.drawString(GAMstr[MENid==0?6:MENid-1], DEVw/2, 16, Graphics.TOP | Graphics.HCENTER);
    //#endif

    
    int MENhOut = (MENh-(6*MENhOp+1))/2; 
    int nivY = MENhUp+4+MENhOut;
    if(MENid==0){ //Contenido del Menú principal        

      //Dibujar fondo en la posición de la opción seleccionada
      MEN_DibRect(g,12, nivY+MENop*MENhOp, DEVw-12, nivY+(MENop+1)*MENhOp, true, 0xFFA6BDD3, 0xFF738AAB);
      MEN_DibRect(g,12, nivY+MENop*MENhOp, 12+MENhOp, nivY+(MENop+1)*MENhOp, false, 0xFF738AAB, 0);
...
//#       int nY=nivY+2;
      ...
      for (int i = 0; i < 6; i++) {
        ...
        //#if CFGtamanyoGfx == 2
            //#if CFGfnt<2
            g.setClip(GAMwLe+44, nY, DEVw-24, MENhOp); //44=12+12+MENhOp+2
            g.drawString(GAMstr[i], GAMwLe+44, nY, Graphics.TOP | Graphics.LEFT);
            //#else 
//#             TXT_DibLinea(g,GAMstr[i], GAMwLe+44, nY, -1);
            //#endif
        //#else
            //#if CFGfnt<2
//#                 g.setClip(DEVw/2-30, nY, DEVw-24,MENhOp);
//#                 g.drawString(GAMstr[i], DEVw/2-30, nY, Graphics.TOP | Graphics.LEFT);
            //#else 
//#             TXT_DibLinea(g,GAMstr[i], DEVw/2-30, nY, -1);
            //#endif
        //#endif
        nY += MENhOp;
      }
      //Dibujar icono combi
      HUD_DibImg(g, 15, (MENhUp+4+MENhOut+MENop*MENhOp)+(MENhOp-NUMPIXELS_TILEY)/2, 71);
    }
    else
    {
      ...
      if (MENid == 1 || MENid== 2 || MENid==3 ){
        ...
        //#if CFGfnt<2 
        g.setClip(12, nivY, DEVw-24, MENhOp*2);  
        g.drawString(GAMstr[27], DEVw / 2, nivY, Graphics.TOP | Graphics.HCENTER);
        g.drawString("del "+GAMstr[MENid-1], DEVw / 2, nivY+MENhOp, Graphics.TOP | Graphics.HCENTER);
        //#else
//#         TXT_DibLinea(g,GAMstr[27], DEVw / 2, nivY, 0);
//#         TXT_DibLinea(g,"del "+GAMstr[MENid-1], DEVw / 2, nivY+MENhOp,0);
        //#endif

        //Dibujar rectangulo fondo en la pos de la subopción seleccionada
        offset = nivY+MENhOp*(MENsubop+3);
        //#if CFGtamanyoGfx == 2
        offset -=3;
        //#else
//#         offset -=2;
        //#endif
        MEN_DibRect(g,12,offset,DEVw-12, offset+MENhOp, true, 0xFFA6BDD3, 0xFF738AAB);
        ...
        //#if CFGfnt<2 
        g.setClip(12, nivY+MENhOp*3, DEVw-24, MENhOp);  
        g.drawString(GAMstr[MENval1 + offset], DEVw / 2, nivY+MENhOp*3, Graphics.TOP | Graphics.HCENTER);        
        //#else
//#         TXT_DibLinea(g,GAMstr[MENval1 + offset], DEVw / 2, nivY+MENhOp*3,0);
        //#endif
        ...
        //#if CFGfnt<2 
        g.setClip(12, nivY+MENhOp*4, DEVw-24, MENhOp);  
        g.drawString(GAMstr[32]+Integer.toString(MENval2+1), DEVw / 2, nivY+MENhOp*4, Graphics.TOP | Graphics.HCENTER);        
        //#else
//#         TXT_DibLinea(g,GAMstr[32]+Integer.toString(MENval2+1), DEVw / 2, nivY+MENhOp*4,0);
        //#endif
        
        //Dibujar flechas
        MEN_DibFlechas(g, MENval1, (byte)(LVL.confFaseMod[MENid-1]-1), 16, nivY+MENhOp*3+2);
        MEN_DibFlechas(g, MENval2, (byte)(LVL.confFaseNivMod[MENid-1][MENval1]-1), 16, nivY+MENhOp*4+2);
        
        //Dibujar Record
        if(babierto){
          if(MENid==3) //Dibujar Records en fases/Niveles abiertos
            HUD_DibTim(g,(short)(nivY+MENhOp*5+4), MENval1, MENval2); //.drawString(MENid==1?GAMstr[32]:GAMstr[33],
          else if (MENid==2)
            HUD_DibPun(g,(short)(nivY+MENhOp*5+4),(byte)1);
        }

      }
      else if (MENid == 4){ //Opciones
        //#if CFGfnt<2 
        g.setClip(12, nivY+MENhOp, DEVw-24, MENhOp);   
        g.drawString(GAMstr[28], DEVw / 2, nivY+MENhOp, Graphics.TOP | Graphics.HCENTER);
        //#else
//#         TXT_DibLinea(g, GAMstr[28], DEVw / 2, nivY+MENhOp, 0);
        //#endif
...
//#         offset -=2;
        ...
        //Dibujar opciones
        //#if CFGfnt<2
        g.setClip(12, nivY+MENhOp*3, DEVw-24, MENhOp*2);   
        g.setFont(fSbt);
        g.setColor(MENsubop==0?0x224466:0xFFFFFF);
        g.drawString(GAMstr[COMBIMidlet.M.MENbMus?33:34], DEVw / 2, nivY+MENhOp*3, Graphics.TOP | Graphics.HCENTER);
        g.setColor(MENsubop==1?0x224466:0xFFFFFF);
        g.drawString(GAMstr[COMBIMidlet.M.MENbSnd?35:36], DEVw / 2, nivY+MENhOp*4, Graphics.TOP | Graphics.HCENTER);
        //#else
//#         TXT_DibLinea(g,GAMstr[COMBIMidlet.M.MENbMus?33:34], DEVw / 2, nivY+MENhOp*3,0);
//#         TXT_DibLinea(g,GAMstr[COMBIMidlet.M.MENbSnd?35:36], DEVw / 2, nivY+MENhOp*4,0);        
        //#endif
      }
      else if (MENid == 5) { //Créditos
        //#if CFGfnt<2 
        g.setClip(12, nivY+MENhOp, DEVw-24, MENhOp*4);   
        g.drawString(GAMstr[49], DEVw / 2, nivY+MENhOp, Graphics.TOP | Graphics.HCENTER);
        g.drawString(GAMstr[50], DEVw / 2, nivY+MENhOp*3, Graphics.TOP | Graphics.HCENTER);        
        g.setColor(0xFFFFFF);
        g.drawString("Roberto Peña", DEVw / 2, nivY+MENhOp*2, Graphics.TOP | Graphics.HCENTER);
        g.drawString("R. Peña y Lourdes Martín", DEVw / 2, nivY+MENhOp*4, Graphics.TOP | Graphics.HCENTER);
        //#else
//#         TXT_DibLinea(g,GAMstr[49], DEVw / 2, nivY+MENhOp,0);
//#         TXT_DibLinea(g,GAMstr[50], DEVw / 2, nivY+MENhOp*3,0);
//#         TXT_DibLinea(g,"Roberto Peña", DEVw / 2, nivY+MENhOp*2,0);
//#         TXT_DibLinea(g,"R. Peña y Lourdes Martín", DEVw / 2, nivY+MENhOp*4,0);
        //#endif
//        g.drawString("Total Memory: "+Long.toString(Runtime.getRuntime().totalMemory()), DEVw / 2, 96, Graphics.TOP | Graphics.HCENTER);
//        g.drawString("Free Memory: "+Long.toString(Runtime.getRuntime().freeMemory()), DEVw / 2, 114, Graphics.TOP | Graphics.HCENTER);
//        g.drawString("Sound Count TONE:" +Sound.getConcurrentSoundCount(Sound.FORMAT_TONE), DEVw / 2, 132, Graphics.TOP | Graphics.HCENTER);
//        g.drawString("Sound Count WAV:" +Sound.getConcurrentSoundCount(Sound.FORMAT_WAV), DEVw / 2, 150, Graphics.TOP | Graphics.HCENTER);
//        g.drawString("MMAPI (JSR 135) vers:"+System.getProperty("microedition.media.version"), DEVw / 2, 132, Graphics.TOP | Graphics.HCENTER);
//        g.drawString("Video capture:"+System.getProperty("supports.video.capture"),DEVw / 2, 150, Graphics.TOP | Graphics.HCENTER);
        ...
        if(MENcheatSts >-1){
          ...
          g.drawString(MENcheatPwd.substring(0, MENcheatSts + 1), DEVw / 2, nivY+MENhOp*5+MENhOut/2, Graphics.TOP | Graphics.HCENTER);
          //#else
//#           TXT_DibLinea(g,MENcheatPwd.substring(0, MENcheatSts + 1), DEVw / 2, nivY+MENhOp*5+MENhOut/2,0);
          //#endif
        }
      }
      else { //MENid == 6
        //#if CFGfnt<2  
        g.setClip(12, nivY+MENhOp, DEVw-24, MENhOp);  
        g.drawString(GAMstr[30], DEVw / 2, nivY+MENhOp, Graphics.TOP | Graphics.HCENTER);
        //#else
//#         TXT_DibLinea(g,GAMstr[30], DEVw / 2, nivY+MENhOp,0);
        //#endif
      }
    }
}
...
private void MEN_DibFlechas(Graphics g, byte pval, byte plim, int px, int py){
    ...
    HUD_DibImg(g, DEVw-px-6, py, pval<plim?28:26);
}

private void MEN_DibKeys(Graphics g, byte pid){
    int i;

    MEN_DibRect(g,0,DEVh-MENhDo,DEVw,DEVh, false, 0xFF336699, 0);
    g.setFont(fCnt);
    g.setColor(0xFFFFFF);
    if((i=MENkeys[pid*2]) != -1){
        //#if CFGfnt<2 
        g.drawString(GAMstr[i],4,DEVh-1, Graphics.BOTTOM | Graphics.LEFT);
       //#else        
//#         TXT_DibLinea(g,GAMstr[i],4,DEVh-1-FNTH,-1); //KKtxt
      //#endif
    }
    if((i=MENkeys[pid*2+1]) != -1){
      //#if CFGfnt<2 
      g.drawString(GAMstr[i],DEVw-4,DEVh-1, Graphics.BOTTOM | Graphics.RIGHT);
      //#else      
//#       TXT_DibLinea(g,GAMstr[i],DEVw-4,DEVh-1-FNTH,1);  //KKtxt
      //#endif
    }
    ...
    if(MENmsg != -1){
      //#if CFGfnt<2         
      g.drawString(GAMstr[MENmsg],DEVw/2,DEVh-1, Graphics.BOTTOM | Graphics.HCENTER);
      //#else      
//#       TXT_DibLinea(g,GAMstr[MENmsg],DEVw/2,DEVh-1-FNTH,0); //KKtxt
      //#endif      
    }
}
...
private void GAM_Dibujar(Graphics g){
   ...
   //System.out.println(" => GAM_Dibujar");
    g.setColor(LVL.colbase);
    g.fillRect(14*NUMPIXELS_TILEX,0,DEVw-14*NUMPIXELS_TILEX,14*NUMPIXELS_TILEY);
...
private void FX_DibMsg(Graphics g, short pidx){
    FX_DibRect(g,0,DEVw,32,52);
    //Texto
    g.setFont(fSbt);
    if(FXdat[pidx][11] == 40){
      g.drawString(GAMstr[FXdat[pidx][11]]+(FXdat[pidx][8]>1?"  (x"+FXdat[pidx][8]+")":""), DEVw / 2 - 20, 36, Graphics.TOP | Graphics.HCENTER);
      g.drawString(Integer.toString(FXdat[pidx][9] + FXdat[pidx][4]) + "%", 140, 36, Graphics.TOP | Graphics.LEFT);
    }else
      g.drawString(GAMstr[FXdat[pidx][11]], DEVw / 2, 36, Graphics.TOP | Graphics.HCENTER);

}

private void FX_DibDialog(Graphics g, short pidx){
    FX_DibRect(g,
               DEVw/2 - FXdat[pidx][4]*NUMPIXELS_TILEX,
               DEVw/2 + FXdat[pidx][4]*NUMPIXELS_TILEX,
               32,
               32+FXdat[pidx][11]);
}
...
private void FX_DibTrans(Graphics g, short pidx){
    ...
    xPoints[1] = xPoints[2] = DEVw;
    yPoints[0] = yPoints[1] = 0;
    yPoints[2] = yPoints[3] = DEVh;
    int mask = 0x00FFFFFF;
    if(FXdat[pidx][11]==0)
...
private void FX_DibContCur(Graphics g, short pidx){
    ...
    //Marco sup/inf
    g.setColor(0xE5A332);
    g.drawLine(DEVw-36,0,DEVw,0);
    g.drawLine(DEVw-36,20,DEVw,20);
    //Decremento contador e icono modo cursor
    HUD_DibImg(g, DEVw-34, 4, 11); //signo -
    HUD_DibImg(g, DEVw-22, 4, 68); //1
    HUD_DibImg(g, DEVw-14, 4, FXdat[pidx][11]!=4?FXdat[pidx][11]+12:19);
}
