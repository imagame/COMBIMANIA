...
//	*************************************************** Variables del juego (GAM)
         ...
         //Variables Menú pausa
           ...
           int MENsit[] = {10 * NUMPIXELS_TILEY-4 , 16 * NUMPIXELS_TILEY-2, //sit 0
           ...
           //Variables graficos
           private Image imgHud, imgCab;
           private Image GAMspr[] = new Image[7];
           private short GAMpiePix[];// = new short[14 * 7];
...
// Carga de Graficos Cursores (matriz de 7x7 tiles)
       if(PIXnum==4){
         for (int j = 0; j < 2; j++) {
           for (int i = 0; i < 8; i++) {
             CURpix[j * 8 + i] = DirectUtils.createImage(NUMPIXELS_TILEX + 8,
                 NUMPIXELS_TILEY + 8, 0x00000000);
             dg = DirectUtils.getDirectGraphics(CURpix[j * 8 + i].getGraphics());
             dg.drawImage(img, - (NUMPIXELS_TILEY + 8) * i,
                          - (NUMPIXELS_TILEY + 8) * j,
                          Graphics.TOP | Graphics.LEFT,
                          0);
           }
       ...
       if(PIXnum==3){
         for (int i = 0; i < 7; ++i) {
           GAMspr[i] = DirectUtils.createImage(12, 12, 0x00000000);
           dg = DirectUtils.getDirectGraphics(GAMspr[i].getGraphics());
           dg.drawImage(img, -12 * i, 0, Graphics.TOP | Graphics.LEFT, 0);
         }
       }
...
private void HUD_DibTut(Graphics g){
    ...
    MENmsg = -1;
    MEN_DibKeys(g,TXTsig?(byte)9:8,false); //LVL.bTut);
    if(TXTsig)
...
private void HUD_SoftLeft(){
    if(GAMbNivIntro){
      ...
      else if (TXTidLinAct < (LVL.TXTstr.length-1))
...
private void TXT_LinUpd(int pInc){
    TXTidLinAct += pInc;
//    if(TUTidLinAct + (NUMLINTXT-1) >= LVL.TXTstr.length-1)
...
//Preinicialización sistema gráfico
       ...
       //[S60]:
       BGRimg1 = Image.createImage(15*NUMPIXELS_TILEX,14*NUMPIXELS_TILEY);
       BGRimg2 = Image.createImage(15*NUMPIXELS_TILEX,14*NUMPIXELS_TILEY);
       //Fin[S60]
...
private void MEN_Dibujar(Graphics g){
    ...
    g.setClip(0, 0, DEVscrPxlW, DEVscrPxlH);
    g.fillRect(0,0,DEVscrPxlW,DEVscrPxlH);
    g.setColor(0x3B465A); //color azul fuerte
    g.drawRect(0,0,DEVscrPxlW-1,DEVscrPxlH-17);
    g.drawRect(4,67,DEVscrPxlW-9,DEVscrPxlH-(68+17+3));
    g.setColor(0xFDAC8E); //Color salmon
    g.fillRect(1,64,3,DEVscrPxlH-81);
    g.fillRect(DEVscrPxlW-4,64,3,DEVscrPxlH-81);
    g.fillRect(3,64,DEVscrPxlW-7,3);
    g.fillRect(3,DEVscrPxlH-20,DEVscrPxlW-7,3);
    //g.setColor(0x3B465A);
    ...
    g.drawImage(imgCab, 1, 1, Graphics.TOP | Graphics.LEFT);
    //Dibujar subcabecera menú

    //Dibujar pie
    MEN_DibKeys(g, MENid, false);

    //Dibujar título submenú
     MEN_DibRect(g,66,42, 171, 62, false, 0xB0336699, 0);
     g.setColor(0xFFFFFF);
     g.setFont(MENid==3?fSbt:fTit);
     g.drawString(GAMstr[MENid==0?6:MENid-1], 120, 46, Graphics.TOP | Graphics.HCENTER);

    if(MENid==0){ //Contenido del Menú principal
      int nivY = 78;
      //Dibujar fondo en la posición de la opción seleccionada
      MEN_DibRect(g,12,nivY+MENop*18-4,DEVscrPxlW-12, nivY+MENop*18+14, true, 0xFFA6BDD3, 0xFF738AAB);
      MEN_DibRect(g,12,nivY+MENop*18-4,30, nivY+MENop*18+14, false, 0xFF738AAB, 0);
      ...
      for (int i = 0; i < 6; i++) {
        ...
        g.drawString(GAMstr[i], 44, nivY, Graphics.TOP | Graphics.LEFT);
        nivY += 18;
      }
      //Dibujar icono combi
      HUD_DibImg(g, 15, 78+MENop*18-1, 71);
    }
    else{
      ...
      if (MENid == 1 || MENid== 2 || MENid==3 ){
        ...
        g.drawString(GAMstr[27], DEVscrPxlW / 2, 78, Graphics.TOP | Graphics.HCENTER);
        g.drawString("del "+GAMstr[MENid-1], DEVscrPxlW / 2, 90, Graphics.TOP | Graphics.HCENTER);


        //Dibujar rectangulo fondo en la pos de la subopción seleccionada
        offset = MENsubop==0?120:138;
        MEN_DibRect(g,12,offset,DEVscrPxlW-12, offset+18, true, 0xFFA6BDD3, 0xFF738AAB);
        ...
        g.drawString(GAMstr[MENval1 + offset], DEVscrPxlW / 2, 124, Graphics.TOP | Graphics.HCENTER);
        ...
        g.drawString(GAMstr[32]+Integer.toString(MENval2+1), DEVscrPxlW / 2, 142, Graphics.TOP | Graphics.HCENTER);

        //Dibujar flechas
        MEN_DibFlechas(g, MENval1, (byte)(LVL.confFaseMod[MENid-1]-1), 27, 126);
        MEN_DibFlechas(g, MENval2, (byte)(LVL.confFaseNivMod[MENid-1][MENval1]-1), 27, 144);
        //Dibujar Record
        if(babierto){
          if(MENid==3) //Dibujar Records en fases/Niveles abiertos
            HUD_DibTim(g,(short)168, MENval1, MENval2); //.drawString(MENid==1?GAMstr[32]:GAMstr[33],
          else if (MENid==2)
            HUD_DibPun(g,(short)168,(byte)1);
        }

      }
      else if (MENid == 4){ //Opciones
        g.drawString(GAMstr[28], DEVscrPxlW / 2, 78, Graphics.TOP | Graphics.HCENTER);
        //Dibujar rectangulo fondo en la pos de la subopción seleccionada
        offset = MENsubop==0?120:138;
        MEN_DibRect(g,12,offset,DEVscrPxlW-12, offset+18, true, 0xFFA6BDD3, 0xFF738AAB);
        //Dibujar opciones
        g.setFont(fSbt);
        g.setColor(MENsubop==0?0x224466:0xFFFFFF);
        g.drawString(GAMstr[COMBIMidlet.M.MENbMus?33:34], DEVscrPxlW / 2, 124, Graphics.TOP | Graphics.HCENTER);
        g.setColor(MENsubop==1?0x224466:0xFFFFFF);
        g.drawString(GAMstr[COMBIMidlet.M.MENbSnd?35:36], DEVscrPxlW / 2, 142, Graphics.TOP | Graphics.HCENTER);
      }
      else if (MENid == 5) { //Créditos
        g.drawString(GAMstr[49], DEVscrPxlW / 2, 78, Graphics.TOP | Graphics.HCENTER);
        g.drawString(GAMstr[50], DEVscrPxlW / 2, 132, Graphics.TOP | Graphics.HCENTER);

        g.setColor(0xFFFFFF);
        g.drawString("Roberto Peña", DEVscrPxlW / 2, 96, Graphics.TOP | Graphics.HCENTER);
        g.drawString("Roberto Peña y Lourdes Martín", DEVscrPxlW / 2, 150, Graphics.TOP | Graphics.HCENTER);
...
//        g.drawString("Video capture:"+System.getProperty("supports.video.capture"),DEVscrPxlW / 2, 150, Graphics.TOP | Graphics.HCENTER);
        ...
        if(MENcheatSts >-1){
          ...
          g.drawString(MENcheatPwd.substring(0, MENcheatSts + 1), DEVscrPxlW / 2, 172, Graphics.TOP | Graphics.HCENTER);
        }
      }
      else  //MENid == 6
        g.drawString(GAMstr[30], DEVscrPxlW / 2, 78, Graphics.TOP | Graphics.HCENTER);
...
private void MEN_DibKeys(Graphics g, byte pid, boolean ptrans){
    int i;

    MEN_DibRect(g,0,DEVscrPxlH-16,DEVscrPxlW,DEVscrPxlH, false, ptrans?0x40336699:0xFF336699, 0);
    g.setFont(fCnt);
    g.setColor(0xFFFFFF);
...
private void BGR_Dibujar(Graphics g){
    ...
    if(BGRprin)
      g.drawImage(BGRimg1, 0, 0, Graphics.TOP | Graphics.LEFT);
    else
      g.drawImage(BGRimg2, 0, 0, Graphics.TOP | Graphics.LEFT);
    ...
    for (int j = 0; j < NUMTILES_SCRY; j++) {
      ...
      if(idPie > -1){ //Si hay pieza
        if(PIEdat[idPie][4] == -1  || (PIEdat[idPie][4] !=-1 && COMdat[PIEdat[idPie][4]][0] == IN_FX)) //y no hay combi  => o hay combi y éste está en estaFX
          BGR_DibujarPartPie(BGRdg, idPie, NIVnumPart[idx], x, y);
        else{
...
//	---------------------------------------------------- BGR_DibScrollRestoCols
private void BGR_DibScrollRestoCols(Graphics BGRgDst, Image BGRimgSrc) {
    if(BGRscroll== -1)
      BGRgDst.drawImage(BGRimgSrc, NUMPIXELS_TILEX, 0, Graphics.TOP | Graphics.LEFT);
    else
      BGRgDst.drawImage(BGRimgSrc, -NUMPIXELS_TILEX, 0, Graphics.TOP | Graphics.LEFT);
}
...
//	--------------------------------------------------- BGR_DibScrollRestoFilas
private void BGR_DibScrollRestoFilas(Graphics BGRgDst, Image BGRimgSrc) {
    if(BGRscroll== -2)
      BGRgDst.drawImage(BGRimgSrc, 0, NUMPIXELS_TILEY, Graphics.TOP | Graphics.LEFT);
    else
      BGRgDst.drawImage(BGRimgSrc, 0, -NUMPIXELS_TILEY, Graphics.TOP | Graphics.LEFT);
}
...
//	--------------------------------------------------------- BGR_DibujarPartPie
private void BGR_DibujarPartPie(DirectGraphics BGRdg, short pIdPie, short pIdPart, short pNivX, short pNivY ) {
    BGRdg.drawPixels(GAMpiePix, false,
                     (PIEdat[pIdPie][3] - 1)* 14 * NUMPIXELS_TILE_XY + (pIdPart-1) * NUMPIXELS_TILE_XY,
                     NUMPIXELS_TILEX,
                     pNivX, pNivY,
